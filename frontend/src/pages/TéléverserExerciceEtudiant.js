...
const TeleverserExerciceEtudiant = () => {
  ...
  useEffect(() => {
    const fetchCours = async () => {
      ...
      try {
        const res = await axios.get('http://195.179.229.230:5004/api/etudiant/mes-cours', {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await axios.post('http://195.179.229.230:5004/api/etudiant/exercices', formData, {
        headers: {
