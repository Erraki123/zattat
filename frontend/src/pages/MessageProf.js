...
const UserAvatar = ({ user, size = 48 }) => {
  ...
  const getImageUrl = (imagePath) => {
    ...
    if (imagePath.startsWith('/')) {
      return `http://195.179.229.230:5004${imagePath}`;
    }
...
const [audioBlob, setAudioBlob] = useState(null);
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5004/api/professeur/me', {
  ...
  useEffect(() => {
    const fetchEtudiants = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5004/api/professeur/mes-etudiants-messages', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    const fetchUnreadCounts = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5004/api/messages/unread-by-sender', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    const checkOnlineStatus = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5004/api/users/online-status', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const response = await fetch(`http://195.179.229.230:5004/api/messages/professeur/${selectedEtudiant._id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        await fetch('http://195.179.229.230:5004/api/messages/mark-conversation-read', {
          method: 'PATCH',
          headers: {
  ...
  const envoyerMessage = async () => {
    ...
    try {
      const response = await fetch('http://195.179.229.230:5004/api/messages/upload-prof', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData
      });
  ...
  const confirmDeleteMessage = async () => {
    ...
    try {
      const response = await fetch(`http://195.179.229.230:5004/api/messages/${messageToDelete}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
...
{selectedEtudiant.nomComplet}
  ...
  <audio controls src={`http://195.179.229.230:5004${message.fichier}`} style={{ marginTop: '8px', width: '100%' }} />
)}
                            ...
                            {message.fichier && (
                              <div style={{
                                ...
                                <a
                                  href={`http://195.179.229.230:5004${message.fichier}`}
                                  target="_blank"
