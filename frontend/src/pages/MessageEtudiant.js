...
const audioChunksRef = useRef([]);
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5004/api/etudiant/me', {
  ...
  useEffect(() => {
    const fetchProfesseurs = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5004/api/etudiant/mes-professeurs-messages', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const response = await fetch(`http://195.179.229.230:5004/api/messages/etudiant/${selectedProf._id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
 ...
 const envoyerMessage = async () => {
  ...
  try {
    const response = await fetch('http://195.179.229.230:5004/api/messages/upload', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: formData
    });
