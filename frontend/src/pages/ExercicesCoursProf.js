...
const ExercicesCoursProf = () => {
  ...
  useEffect(() => {
    const fetchExercices = async () => {
      try {
        ...
        const res = await axios.get(`http://195.179.229.230:5004/api/professeur/exercices/${nomCours}`, {
  ...
  const saveRemarque = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://195.179.229.230:5004/api/professeur/exercices/${id}/remarque`, {
        remarque
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
