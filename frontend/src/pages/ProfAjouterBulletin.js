...
const ProfAjouterBulletin = () => {
  ...
  useEffect(() => {
    const fetchCours = async () => {
      try {
        ...
        const res = await fetch('http://195.179.229.230:5004/api/professeur/mes-cours', {
  ...
  useEffect(() => {
    const fetchBulletins = async () => {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch('http://195.179.229.230:5004/api/bulletins/professeur', {
          headers: { Authorization: `Bearer ${token}` }
        });
        const data = await res.json();
        setBulletins(data);
  ...
  useEffect(() => {
    ...
    const fetchEtudiants = async () => {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch('http://195.179.229.230:5004/api/professeur/etudiants', {
          headers: { Authorization: `Bearer ${token}` }
        });
        const data = await res.json();
  ...
  const handleSubmitAll = async () => {
    ...
    try {
        ...
        if (evaluation.titre.trim() && evaluation.coefficient) {
          ...
          Object.keys(etudiantsNotes).forEach(etudiantId => {
            ...
            if (note && note.trim() !== '') {
              ...
              promises.push(
                fetch('http://195.179.229.230:5004/api/bulletins', {
                  method: 'POST',
                  headers: {
      ...
      const bulletinsRes = await fetch('http://195.179.229.230:5004/api/bulletins/professeur', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDeleteGroup = async (group) => {
    ...
    try {
      ...
      const promises = group.etudiants.map(etudiant => 
        fetch(`http://195.179.229.230:5004/api/bulletins/${etudiant._id}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` }
        })
      );
