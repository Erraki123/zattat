...
const Calendrier = () => {
  ...
  useEffect(() => {
    const fetchEvenements = async () => {
      try {
        ...
        const res = await axios.get('http://195.179.229.230:5004/api/evenements', {
  ...
  const handleDeleteEvent = async (event) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer l'événement "${event.titre}" ?`)) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`http://195.179.229.230:5004/api/evenements/${event._id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const saveEditedEvent = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.put(`http://195.179.229.230:5004/api/evenements/${editingEvent._id}`, {
  ...
  const addNewEvent = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post('http://195.179.229.230:5004/api/evenements', {
